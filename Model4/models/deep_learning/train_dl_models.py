"""Train and evaluate Model4 deep-learning models on generated sequence data."""
...
from Model4.models.deep_learning.dl_model_architectures import get_model_builders  # noqa: E402
from Model4.models.deep_learning.dl_utils import (  # noqa: E402
    build_callbacks,
    evaluate_dl_model,
...
LOGGER = logging.getLogger("Model4DeepLearning")


def load_boosting_summary() -> Dict[str, Dict[str, float]]:
    """Load existing Model4 boosting metrics if available."""
...
def train_deep_learning_models(epochs: int = 30, batch_size: int = 64) -> Dict[str, Dict[str, float]]:
    """Train the baseline deep-learning family for Model4."""
    ensure_model4_directories()
    set_global_seed()

    if not SEQUENCE_DATASET_PATH.exists():
        LOGGER.info("Sequence dataset missing. Generating Model4 sequence artifact first.")
    ...
    model_files = {
        "CNN": "cnn_model4.h5",
        "LSTM": "lstm_model4.h5",
        "CNN-LSTM": "cnn_lstm_model4.h5",
        "CNN-BiLSTM": "cnn_bilstm_model4.h5",
        "CNN-Attention-LSTM": "cnn_attention_lstm_model4.h5",
    }
    ...
    plot_roc_curves(roc_inputs, split_bundle["y_test"], PLOTS_DIR / "roc_curves_dl_models.png", "Model4 Deep Learning ROC Curves")
    plot_multi_confusion_matrices(
        confusion_inputs,
        split_bundle["y_test"],
    ...
    plot_metric_comparison(metrics_only, PLOTS_DIR / "model_comparison_dl.png", "Model4 Deep Learning Model Comparison")
    plot_classwise_metric_heatmap(
        report_only,
        PLOTS_DIR / "classwise_f1_dl.png",
        metric_name="f1-score",
        title="Deep Learning Class-wise F1 Scores",
    )

    boosting_summary = load_boosting_summary()
    if boosting_summary:
        combined = {**boosting_summary, **metrics_only}
        plot_metric_comparison(combined, PLOTS_DIR / "model_comparison_combined.png", "Model4 Boosting vs Deep Learning")
    return metrics_only


def main() -> None:
    """Entry point for Model4 deep-learning training."""
