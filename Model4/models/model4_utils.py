"""Shared utilities for Model4 training, plotting, and reporting."""
...
from Model4.models.model4_config import (
...
def ensure_model4_directories() -> None:
    """Create the standard Model4 directory tree."""
    for path in [OUTPUT_DIR, PLOTS_DIR, REPORTS_DIR, BOOSTING_MODELS_DIR, DL_MODELS_DIR, SEQUENCE_DIR]:
...
def load_and_validate_dataset(dataset_path: Path = DATASET_PATH) -> Tuple[pd.DataFrame, Dict[str, object]]:
    """Load the Model4 dataset and validate schema, missingness, and balance."""
    if not dataset_path.exists():
        raise FileNotFoundError(f"Model4 dataset not found: {dataset_path}")
    ...
    LOGGER.info("Validated Model4 dataset: %s rows, %s columns", df.shape[0], df.shape[1])
    return df, report
...
def plot_class_distribution(df: pd.DataFrame, output_path: Path) -> None:
    """Save the class distribution plot for the Model4 dataset."""
    ...
    ax.set_title("Model4 Dataset Class Distribution")
    ax.set_xlabel("Class")
    ax.set_ylabel("Count")
    fig.tight_layout()
...
def generate_report_markdown(
    ...
    """Generate the Model4 markdown report."""
    ...
    return f"""# Model4 Experiment Summary
...
- Deep learning family: `CNN`, `LSTM`, `CNN-LSTM`, `CNN-BiLSTM`, `CNN-Attention-LSTM`
...
## Artifact Locations
- Metrics JSON/CSV: `Model4/outputs`
- Trained boosting models: `Model4/outputs/boosting_models`
- Trained deep learning models: `Model4/outputs/deep_learning_models`
